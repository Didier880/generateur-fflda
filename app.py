    ...
    if fichier_resultats is not None:
        try:
            ...
            for nom_feuille in onglets_poules:
                ...
                # Détection automatique de la ligne d'en-tête et des colonnes dans la feuille Excel
                header_row = 4
                nom_col_idx, club_col_idx, total_pts_col_idx, poids_col_idx = 3, 4, None, None
                ...
                for col in range(1, ws.max_column + 1):
                    ...
                    if "nom" in val:
                        ...
                        club_col_idx = col
                    elif "total pts" in val or "total" in val:
                        total_pts_col_idx = col
                    elif "poids" in val:
                        poids_col_idx = col

                # Fallback par défaut si non trouvé précisément
                if total_pts_col_idx is None:
                    total_pts_col_idx = ws.max_column - 2
                if poids_col_idx is None:
                    poids_col_idx = ws.max_column

                r = header_row + 1
                lutteurs_poule = []
                while ws.cell(row=r, column=nom_col_idx).value is not None:
                    nom = ws.cell(row=r, column=nom_col_idx).value
                    club = ws.cell(row=r, column=club_col_idx).value if club_col_idx else "Indépendant"
                    total_pts = ws.cell(row=r, column=total_pts_col_idx).value 
                    poids = ws.cell(row=r, column=poids_col_idx).value if poids_col_idx else "0"
            ...
            if not tous_les_resultats:
                st.warning("⚠️ Aucun résultat exploitable n'a été trouvé dans les feuilles de poules.")
                st.stop()
            ...
            with pd.ExcelWriter(output_bilan, engine='openpyxl') as writer:
                ...
                # --- CONFIGURATION PRO POUR L'IMPRESSION SANS COUPURE ---
